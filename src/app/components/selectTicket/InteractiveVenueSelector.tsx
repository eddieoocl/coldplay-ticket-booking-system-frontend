...
export default function InteractiveVenueSelector({ onSectionSelect, selectedTickets }: InteractiveVenueSelectorProps) {
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-lg shadow-xl">
            <h2 className="text-2xl font-bold mb-4 text-center text-yellow-400">选择区域</h2>
            <div className="relative w-full aspect-square overflow-hidden">
                ...
                >
                    ...
                    >
                        {/* Stage */}
                        <rect x="400" y="400" width="200" height="100" fill="#333" />
                        <text x="500" y="460" textAnchor="middle" fill="white" className="text-sm">STAGE</text>

                        {/* Sections */}
                        {sections.map((section) => {
                            const isSelected = selectedTickets.some(ticket => ticket.sectionId === section.id)
                            const sectionColor = priceColors[section.price] || '#999'
                                ...
                                >
                                    {section.subSections.map((subSection) => (
                                        <path
                                            ...
                                            d={subSection.path}
                                            fill={
                                                !section.available ? '#666' :
                                                    isSelected ? '#4CAF50' :
                                                        sectionColor
                                            }
                                            stroke="#000"
                                            strokeWidth="1"
                                            opacity={section.available ? 0.8 : 0.4}
                ...
                {hoveredSection && (
                    ...
                    >
                        ...
                        <p>价格: ¥{hoveredSection.price}</p>
                        <p>剩余: {hoveredSection.quantity}</p>
                    </div>
                )}
            ...
            {/* Section Info */}
            <div className="mt-4">
                <h3 className="text-lg font-semibold mb-2 text-blue-300">区域信息</h3>
                {currentSection ? (
                    <div className="text-white">
                        <p>选中区域: {currentSection.name}</p>
                        <p>价格: ¥{currentSection.price}</p>
                        <p>剩余数量: {currentSection.quantity}</p>
                    </div>
                ) : (
                    <p className="text-gray-400">请选择一个区域查看详情</p>
                )}
            </div>
        </div>
    )
...
function generateSubSections(sectionIndex: number, totalSections: number, subSectionsPerSection: number): SubSection[] {
...
function getCenterX(path: string): number {
    ...
    const sum = matches.reduce((acc, match) => acc + parseFloat(match.split(/\s+/)[1]), 0);
...
function getCenterY(path: string): number {
    ...
    if (!matches) return 500;
    const sum = matches.reduce((acc, match) => acc + parseFloat(match.split(/\s+/)[2]), 0);
    return preciseRound(sum / matches.length, 6);
}
