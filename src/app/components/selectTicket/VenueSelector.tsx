...
const areas: Area[] = [
    { id: 'vip', name: 'VIP区', capacity: 100, price: 1000, color: 'bg-red-500' },
...
export default function VenueSelector({ onAreaSelect }: { onAreaSelect: (area: Area | null) => void }) {
                ...
                {areas.map((area, index) => (
                    <button
                        ...
                        className={`absolute w-1/5 h-1/5 ${area.color} rounded-full flex items-center justify-center text-white font-bold ${
                            selectedArea?.id === area.id ? 'ring-4 ring-white' : ''
                        }`}
                        style={{
            ...
            <div className="mt-4">
                ...
                {selectedArea ? (
                    <div className="text-white">
                        <p>选中区域: {selectedArea.name}</p>
                        <p>容量: {selectedArea.capacity}人</p>
                        <p>价格: ¥{selectedArea.price}</p>
                    </div>
                ) : (
